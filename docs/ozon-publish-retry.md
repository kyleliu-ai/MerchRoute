...
- 当导入错误为 `SPU_ALREADY_EXISTS_IN_ANOTHER_ACCOUNT` 时，将其分类为永久的“商品卡重复”阻断，不再把自动重试当作可执行恢复方案。
- 页面显示失败 Offer、平台错误以及从平台消息中识别出的冲突 Offer；操作员可先执行只读平台状态同步，再决定是否终止本地自动流程。
- 平台状态同步使用 `rowVersion + requestId` 做并发与幂等保护，回读每个变体的上架、归档、库存和平台标识；部分变体成功时保留成功事实，不把整组误报为重新可重试。
- “取消自动任务”只允许处理已有永久导入错误、无运行租约、无活动重试、无 UNKNOWN 网关请求的自动 publication。操作仅关闭 MerchRoute 本地自动流程，不调用 OZON 写接口、不删除平台商品卡，也不清除原失败证据和变体对账结果。
- 测试只使用 mock API 和隔离 PostgreSQL schema，不创建商品卡、不上传媒体、不更新价格或库存。
