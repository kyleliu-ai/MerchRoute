...
`npm run dev` 不加载项目 `.env` 或生产环境文件；只使用登记的开发连接、合成媒体和临时测试密钥。前端 5173 仅代理 4184，E2E 4183，生产 4173。端口占用直接失败。开发进程默认阻断 fetch/http/https 外部业务调用，不启动 n8n；真实联网功能需要独立授权测试，不能临时去掉防护来验收。
