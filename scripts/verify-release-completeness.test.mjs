...
test('历史来源和原有功能完整保留，追加 WB 受控重试', () => {
  ...
  assert.deepEqual(manifest.features.filter((feature) => feature.action === 'INTEGRATE').map((feature) => feature.id).sort(),
    ['local-import-directory-status', 'project-release-guardrails', 'review-delivery-reliability', 'wb-controlled-publish-retry', 'wb-restart-protection']);
  assert.equal(manifest.features.length, 14);
...
test('两种模式共用不可删减的 14 项关键功能与本机 11 类检查契约', () => {
