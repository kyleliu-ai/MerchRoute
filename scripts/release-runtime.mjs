...
import { preflightRuntimeEndpoint, runtimeEndpointFromBinding } from './lib/runtime-endpoint.mjs';

export async function verifyBinding(binding) {
  const endpoint=runtimeEndpointFromBinding(binding,{allowLegacy:true});
  if(!path.isAbsolute(binding.root)||!path.isAbsolute(binding.nodePath)
  ...
  return {...verified,runtimeEndpoint:endpoint};
}
...
export async function startBoundRelease(binding) {
  const verified=await verifyBinding(binding);
  await preflightRuntimeEndpoint(verified.runtimeEndpoint);
  ...
  try{
    ...
    child.unref();return {pid:child.pid,root:binding.root,runtimeEndpoint:verified.runtimeEndpoint};
