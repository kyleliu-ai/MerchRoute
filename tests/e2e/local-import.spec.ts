...
test.describe.serial('E000 local import and E001 delivery', () => {
  ...
  test('imports multiple same-platform media folders through the configurable E000 source', async ({ page }) => {
    ...
    expect(directoryPayload.directories.map((item) => item.importStatus)).toEqual(['NEW', 'NEW']);
    ...
    await page.goto('/purchases/local-import?view=import');
    await page.locator('.local-directory-row.is-platform-root-row').filter({ hasText: 'PDD' }).getByRole('button', { name: '导入产品媒体' }).click();
    const importedRows = page.locator('.local-directory-row.is-product-media-row');
