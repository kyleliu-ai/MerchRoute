...
export class VariantMediaDeliveryService {
  ...
  async deliver(input: DeliveryInput): Promise<SubmissionRecord> {
    ...
    const submissionId = input.retry?.submissionId || randomUUID();
    ...
    try {
      const stageAndCommit = async (commitLock: (operation: () => Promise<void>) => Promise<void>) => {
        ...
        const staged = await this.stageFiles(input, stagedMediaRoot, variantName, kind, submissionId);
        await commitLock(async () => {
          await mkdir(path.dirname(destination), { recursive: true });
          const existing = await stat(destination).catch(() => undefined);
          if (!existing) await rename(stagedMediaRoot, destination);
          else if (!existing.isDirectory()) throw new AppError('TARGET_FOLDER_EXISTS', `${input.platform} 共享媒体目标路径已存在且不是目录`, { destination });
          const committed = await this.inspectCommittedFiles(destination, outputRoot, input, kind, submissionId, staged);
          if (staged.length !== committed.length || staged.some((item, index) => item.sha256 !== committed[index]?.sha256 || item.relativePath !== committed[index]?.relativePath || item.sortOrder !== committed[index]?.sortOrder)) {
            throw new AppError('VERIFY_FAILED', `${input.platform} 共享媒体目标文件校验失败`, { destination });
          }
          const manifestPath = path.join(outputRoot, 'variant-media-manifest.json');
          await this.updateManifest(manifestPath, input, committed);
          record.mediaManifestPath = manifestPath;
        });
      };
      if (input.platform === 'WB') {
        await stageAndCommit((operation) => withWbSourceMediaSkuLock(input.productSku, operation));
      } else {
        const ozonRootDirectory = parseOzonMediaOutputRootTemplate(outputRootTemplate).rootDirectory;
        await withOzonSourceMediaSkuLock(input.productSku, () => withOzonSourceMediaFilesystemLock(
          ozonRootDirectory,
          input.productSku,
          () => stageAndCommit((operation) => this.withSkuLock(`${input.platform}:${input.productSku}`, operation))
        ));
      }
      if (input.archiveMedia !== false && input.stage.approvedArchiveRoot) {
        const archive = path.join(input.stage.approvedArchiveRoot, `${input.task.sourceFolderName}-${submissionId}`);
        await copyDirectory(destination, archive);
        record.archiveFolder = archive;
      }
      ...
      record.errorCode = error instanceof AppError ? error.code : 'COPY_FAILED';
      record.errorMessage = error instanceof Error ? error.message : `${input.platform} 共享媒体投递失败`;
      record.completedAt = new Date().toISOString();
