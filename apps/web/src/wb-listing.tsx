...
function manualPublicationReasonCell(row: WbManualListingRow) {
  ...
  return <div className={`wb-current-reason wb-manual-reason${active ? ' has-progress' : attention ? ' has-error' : ''}`}>
...
function WbListingRegistry({
  ...
  return <div className="wb-registry">
    <Card className="wb-workspace-console wb-manual-console">
      ...
      { title: '绑定预设', width: 190, render: (_, row) => row.publication.presetName || row.publication.presetId ? <div className="wb-current-preset wb-manual-preset"><Space size={4} wrap><strong>{row.publication.presetName || '历史预设快照'}</strong>{row.publication.sourcePresetExists === false && <Tag color="default">原预设已删除</Tag>}</Space><Text type="secondary">R{row.publication.presetRowVersion || '—'} · 已锁定快照</Text></div> : <Text type="secondary">历史任务未记录预设名称</Text> },
