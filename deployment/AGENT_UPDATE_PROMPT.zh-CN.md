...
本提示词对应当前发布快照：Node.js `22.23.1`、npm `10.9.8`、n8n `2.32.6`、PostgreSQL `18.4`、Playwright `1.61.1`、Jimeng `1.0.1`；n8n 清单为 36 个唯一工作流和 3 个部署包；数据库映射为 `merchroute` → `merchroute_app`、`merchroute_n8n` → `merchroute_n8n`。
...
目标包含 v0.1.7 的受控上品重试时，必须同时执行 `docs/releases/v0.1.7.md` 的应用、040/041 迁移及 WB S001 协议核验。切换前检查 WB/OZON 重试台账和租约；不得仅升级页面、直接覆盖 live 工作流或通过真实商品重试验收。新入口默认关闭，只有当前任务授权且运行提交与四个节点代码一致才允许开启；原本停用的工作流继续停用。
...
使用当前安装实际采用的 `launchd`、终端进程或服务管理方式向 MerchRoute 和 n8n 发送正常终止信号；不得直接 `kill -9`。等待发布绑定的 MerchRoute 端口和 `5678` 释放。Jimeng/PostgreSQL 可保持运行，除非升级步骤需要重建。
...
http://127.0.0.1:43173/api/v1/health
