...
- 本批产品 `0.1.4` 是发布工具修复候选版本；`v0.1.2` 与 `v0.1.3` 保留为已发布但本机未激活版本，禁止改写其标签或资产。回读目标 Release 是否真正发布，不能把 package.json、PR 合并或版本较新当成发布完成。
...
安装 v0.1.7 或后续包含受控上品重试的版本时，按 `docs/releases/v0.1.7.md` 回读 040/041 迁移和 WB S001 协议。36 个工作流仍全部以停用状态导入，WB 重试入口默认关闭。不能为验证按钮而启用工作流或操作真实商品；最终报告区分“配套代码已安装”和“业务入口已获准开启”。
